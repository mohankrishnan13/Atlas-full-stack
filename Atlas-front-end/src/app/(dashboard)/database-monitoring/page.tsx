...
export default function DatabaseMonitoringPage() {
  ...
  const criticalDataExport = data.criticalDataExport || { application_name: 'N/A', bytes_exported: 0 };
  const connectionPool = data.connectionPool || { application_name: 'N/A', utilization_percent: 0, avg_query_latency_ms: 0 };

  const exfilData = (Array.isArray(data.dlpByTargetApp) ? data.dlpByTargetApp : [])
    .map(d => ({ ...d, bytes_exported: Number(d.bytes_exported) || 0 }))
    .sort((a, b) => b.bytes_exported - a.bytes_exported)
    .slice(0, 6);

  const suspiciousQueriesData = (Array.isArray(data.operationsByApp) ? data.operationsByApp : [])
    .map(op => ({ 
        name: op.app_name,
  ...
  const safeSuspiciousActivity = Array.isArray(data.suspiciousActivity) ? data.suspiciousActivity : [];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-slate-900 border border-red-900/50 rounded-lg p-5 flex flex-col justify-between shadow-sm">
            ...
            <div className="text-sm text-red-400 font-medium">{( (Number(criticalDataExport.bytes_exported) || 0) / 1e9).toFixed(2)} GB outbound</div>
        ...
        <div className="bg-slate-900 border border-orange-900/50 rounded-lg p-5 flex flex-col justify-between">
            ...
            <div className="text-sm text-orange-400 font-medium">{Number(connectionPool.utilization_percent) || 0}% Capacity | {Number(connectionPool.avg_query_latency_ms) || 0}ms avg latency</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-5">
          ...
          {suspiciousQueriesData.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart layout="vertical" data={suspiciousQueriesData} margin={{ top: 5, right: 20, left: 100, bottom: 20 }}>
