...
import {
  Network, Wifi, AlertTriangle, Info, LoaderCircle,
} from 'lucide-react';
...
const SectionHeader = React.memo(({
  ...
  tooltip: string; right?: React.ReactNode;
}) => (
  <div className="mb-5">
...
const KpiCard = React.memo(({
  value, label, subtitle, color = 'default',
}: {
  value: string | number; label: string; subtitle: string;
...
const AnomalyRow = ({
  ...
  row: NetworkAnomaly;
  onBlockIp: (sourceIp: string, app: string) => void;
}) => (
  <tr className="hover:bg-slate-800/30 transition-colors">
    <td className="px-5 py-4">
      <div className="text-slate-100 font-mono text-xs font-semibold">{row.sourceIp || 'N/A'}</div>
    </td>
    <td className="px-5 py-4">
      <span className="text-blue-400 text-xs font-semibold">{row.app || 'N/A'}</span>
    </td>
    <td className="px-5 py-4">
      <span className="text-slate-400 font-mono text-xs">{row.port?.toString() || 'N/A'}</span>
    </td>
    <td className="px-5 py-4">
      <div className="flex items-center gap-2">
...
export default function NetworkTrafficPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    // Endpoint matches backend route: GET /network-traffic
    apiGet<NetworkTrafficData>('/network-traffic', controller.signal)
      .then(setData)
      .catch((err) => {
  ...
  const handleBlockIp = async (sourceIp: string, app: string) => {
    ...
    try {
      // Matches backend NetworkBlockRequest: { sourceIp, app }
      await apiPost('/network-traffic/block', { sourceIp, app });
      toast.success('Network Block Applied', {
        description: `Hard block applied for ${sourceIp} → ${app}.`,
      });
    } catch (err) {
  ...
  const { safeBandwidth, safeActiveConnections, safeDroppedPackets, safeAnomalies } =
    useMemo(() => {
      const netData = data || {};
    ...
    <div className="space-y-6 pb-8">
      <div>
        ...
        <p className="text-xs text-slate-500 mt-0.5 ml-7">
          Real-time network flow monitoring, bandwidth consumption, and anomaly detection.
        </p>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
        <div className="p-5 border-b border-slate-800">
          <SectionHeader
            ...
            right={
              <span className="text-xs text-slate-500">{safeAnomalies.length} active events</span>
            }
          />
        </div>
