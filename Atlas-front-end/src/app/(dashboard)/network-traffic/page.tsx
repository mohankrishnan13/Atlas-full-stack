...
import {
  ...
  ShieldCheck, Ban, Network, Wifi, AlertTriangle, Info, LoaderCircle
} from 'lucide-react';
...
function KpiCard({ icon, title, subtitle, tooltip, value, valueColor, meta, action }: { icon: React.ReactNode; title: string; subtitle: string; tooltip: string; value: string; valueColor: string; meta?: string; action?: React.ReactNode; }) {
    return (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex flex-col justify-between h-full">
            <div>
                <div className="flex items-center gap-2 mb-1"><div className="text-xs text-slate-400 font-medium uppercase tracking-wide flex items-center gap-2">{icon}{title}</div><InfoTooltip text={tooltip} /></div>
                <p className="text-[11px] text-slate-500 mb-3 leading-relaxed pl-6">{subtitle}</p>
...
function AnomalyBadge({ type }: { type: string }) {
    ...
    const isHigh = isCrit || lower.includes('inject') || lower.includes('scan') || lower.includes('brute');
...
export default function NetworkTrafficPage() {
  ...
  const safeAnomalies = Array.isArray(data?.networkAnomalies) ? data.networkAnomalies : [];
  const internalTargets = [...new Set(safeAnomalies.map((a) => a.app) ?? [])].slice(0, 3);
  ...
  }, [safeAnomalies.length, internalTargets.length]);
  ...
  useEffect(() => {
    const handleResize = () => setTimeout(calculateAndSetPaths, 150);
  ...
  useEffect(() => {
    if (data) {
      setTimeout(calculateAndSetPaths, 150);
    }
  ...
  const highestBandwidthAnomaly = safeAnomalies[0];
  const packetLossAnomaly = safeAnomalies.find((a) => (a.type || '').toLowerCase().includes('spike'));
  const unknownMacs = data.unauthorizedMacs ? (Number(data.unauthorizedMacs.count) || 0) : 0;
  
  const bandwidthGBs = (safeBandwidth / 1024).toFixed(2);
  const droppedPct = safeActive > 0 ? ((safeDropped / (safeActive + safeDropped)) * 100).toFixed(1) : '0.0';

  const firstAnomaly = safeAnomalies[0];

  return (
    <div className="space-y-6 pb-8">
      <div>
        <h1 className="text-xl font-bold text-slate-100 flex items-center gap-2"><Network className="w-5 h-5 text-blue-400" />Network Traffic</h1>
        <p className="text-xs text-slate-500 mt-0.5 ml-7">Real-time network flow monitoring, bandwidth consumption, and unauthorized access detection.</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        <KpiCard icon={<Activity className="w-4 h-4 text-red-500" />} title="Highest Bandwidth Consumer" subtitle="Top source of network bandwidth usage." tooltip="A single source using disproportionate bandwidth can degrade service or indicate data exfiltration." value={highestBandwidthAnomaly?.app ?? 'No anomalies'} valueColor="text-red-400" meta={`${bandwidthGBs} GB/s total active bandwidth`} action={
            <button onClick={() => highestBandwidthAnomaly && handleBlockIp(highestBandwidthAnomaly.sourceIp, highestBandwidthAnomaly.app)} disabled={!highestBandwidthAnomaly} className="w-fit px-4 py-1.5 text-xs font-bold text-orange-300 border border-orange-500/30 rounded-lg hover:bg-orange-900/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">Throttle Bandwidth</button>}
        />
        <KpiCard icon={<Zap className="w-4 h-4 text-orange-500" />} title="Critical Packet Loss" subtitle="App or network layer with highest packet drop rate." tooltip="High packet loss (>1%) causes slowdowns. Often indicates network congestion or hardware issues." value={packetLossAnomaly?.app ?? (safeDropped > 100 ? 'Network Layer' : 'Normal Range')} valueColor="text-orange-400" meta={`${safeDropped.toLocaleString()} dropped packets/min (${droppedPct}% of total)`} />
        <KpiCard icon={<ShieldAlert className="w-4 h-4 text-red-500" />} title="Unauthorized Network Access" subtitle="Unregistered devices on internal network segments." tooltip="Unknown MAC addresses are unregistered devices. They should be blocked immediately to prevent lateral movement." value={`${unknownMacs} Unknown MACs`} valueColor="text-red-400" meta={data.unauthorizedMacs ? `On ${data.unauthorizedMacs.subnet} subnet` : 'No unauthorized devices'} action={
            <button onClick={() => toast.info('MAC address block initiated for unauthorized devices')} disabled={unknownMacs === 0} className="w-fit px-4 py-1.5 text-xs font-bold text-red-300 border border-red-600/40 rounded-lg hover:bg-red-900/30 transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"><Ban className="w-3 h-3" />Block MAC Addresses</button>}
        />
      </div>

      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        <SectionHeader icon={<Network className="w-4 h-4 text-blue-400" />} title="Traffic Flow Network Map" subtitle="Visual representation of live network connections from external sources to internal targets." tooltip="This diagram shows where traffic originates and its destination. Red dashed lines indicate flagged, suspicious flows." />
        <div ref={containerRef} className="relative min-h-[420px] md:min-h-[380px] bg-slate-950/50 rounded-xl border border-slate-800/50 overflow-hidden">
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none z-0">
            ...
            {(internalTargets || []).map((target, idx) => {
              ...
              if (!path) return null;
              const hasAnomaly = safeAnomalies.some(a => a.app === target);
              const color = hasAnomaly ? "#ef4444" : "#10b981";
              const marker = hasAnomaly ? "url(#arrowRed)" : "url(#arrowGreen)";
              return <path key={idx} d={path} fill="none" stroke={color} strokeWidth="2" strokeDasharray={hasAnomaly ? "5,5" : undefined} markerEnd={marker} />
            })}
          </svg>
          ...
          <div className="relative z-10 w-full h-full flex flex-col md:flex-row justify-between items-center px-8 py-4 md:py-0 gap-8 md:gap-0">
            <div className="flex flex-col gap-10 w-full md:w-auto">
              {firstAnomaly && (
                <div id="external-anomaly-node" ref={externalAnomalyRef} className="bg-slate-900 border border-red-500/50 p-3 rounded-lg w-full md:w-60 hover:border-red-500 transition-all shadow-[0_0_15px_rgba(239,68,68,0.1)]">
                  <div className="flex items-center gap-2 mb-1.5"><Globe className="w-4 h-4 text-red-500" /><span className="text-red-400 font-mono text-xs truncate">{firstAnomaly.sourceIp}</span></div>
                  <div className="text-[10px] text-slate-400 mb-1.5">Targeting: <span className="text-slate-200 font-medium">{firstAnomaly.app}</span></div>
                  <div className="flex items-center justify-between"><span className="text-xs text-red-400 font-medium">⚠ Suspicious Activity</span><button onClick={() => handleBlockIp(firstAnomaly.sourceIp, firstAnomaly.app)} className="text-xs flex items-center gap-1 text-red-400 hover:text-red-300 hover:bg-red-900/20 px-1.5 py-0.5 rounded transition-colors border border-transparent hover:border-red-500/30"><ShieldAlert className="w-3 h-3" />Block</button></div>
                </div>
              )}
              <div id="external-authorized-node" ref={externalAuthorizedRef} className="bg-slate-900 border border-blue-500/30 p-3 rounded-lg w-full md:w-60">
                ...
                <div className="text-xs text-slate-500">{(safeActive || 0).toLocaleString()} active sessions</div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-5 w-full md:w-auto">
              {(internalTargets.length > 0 ? internalTargets : ['Internal Network']).map((target, idx) => {
                const hasAnomaly = safeAnomalies.some(a => a.app === target);
                const colorClass = hasAnomaly ? 'red' : 'emerald';
                const statusText = hasAnomaly ? '🔴 CRITICAL' : '✓ HEALTHY';
                const subText = hasAnomaly ? 'Receiving suspicious traffic' : 'Normal parameters';
                ...
                return (
                  <div id={`internal-target-node-${idx}`} ref={el => internalTargetRefs.current[idx] = el} key={idx} className={`bg-slate-900 border p-3 rounded-lg w-full md:w-60 border-${colorClass}-500/${hasAnomaly ? '100' : '30'} ${hasAnomaly ? 'shadow-[0_0_15px_rgba(239,68,68,0.15)]':''}`}>
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className={`text-[10px] px-1.5 py-0.5 rounded border font-semibold bg-${colorClass}-900/50 text-${colorClass}-400 border-${colorClass}-500/30`}>{statusText}</span>
                    </div>
                    <div className="text-[10px] text-slate-500 mb-1.5">{subText}</div>
                  </div>
                );
              })}
            </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
        <div className="p-5 border-b border-slate-800">
          <SectionHeader icon={<Wifi className="w-4 h-4 text-red-500 animate-pulse" />} title="Active Network Anomalies" subtitle="Real-time log of suspicious network connections detected by the IDS/IPS system." tooltip="Use 'Block Source' to immediately drop all traffic from a suspicious IP." right={<span className="text-xs text-slate-500">{safeAnomalies.length} active events</span>} />
        </div>
          ...
          <table className="w-full text-left text-sm min-w-[700px]">
            ...
            <tbody className="divide-y divide-slate-800">
              {safeAnomalies.length > 0 ? (
                safeAnomalies.map((row: NetworkAnomaly) => (
