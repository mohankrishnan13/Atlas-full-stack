...
import {
  Shield, Server, TrendingUp, AlertTriangle, Info, LoaderCircle, Sparkles
} from 'lucide-react';
import {
...
const AppHealthCard = React.memo(({
  appName, load, status, onAction
}: { appName: string; load: string; status: string; onAction: () => void }) => {
  ...
  const c = isCritical ? cfg.critical : cfg.healthy;
  return (
    <div className={`bg-slate-900 border rounded-xl p-4 flex flex-col gap-3 ${c.border} h-full`}>
      <div className="flex items-center justify-between">
        <div className="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">Target Application</div>
        <span className={`text-[10px] px-2 py-0.5 rounded border font-medium ${c.badge}`}>{c.label}</span>
      </div>
      <div className="text-sm font-bold text-slate-100 truncate" title={appName}>{appName}</div>
      <div className="flex-grow">
        <div className="text-xl font-bold text-slate-100 leading-tight">{load}</div>
        <div className="text-[11px] text-slate-500 mt-0.5">Current Load</div>
      </div>
      <button
        onClick={onAction}
        className={`w-full py-2 text-[11px] font-bold rounded-lg transition-colors ${c.btn}`}
      >
        {isCritical ? 'APPLY HARD LIMIT' : 'VIEW TRAFFIC'}
      </button>
    </div>
  );
});
...
const CustomTooltipContent = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-slate-800 border border-slate-600 rounded-lg p-3 text-xs shadow-xl">
      ...
      <p className="text-slate-300 mt-1">{`${payload[0].name}: ${Number(payload[0].value || 0).toLocaleString()}`}</p>
    </div>
  );
};
...
export default function OverviewPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    apiGet<OverviewData>('/overview', controller.signal)
      .then(setData)
      .catch((err) => {
  ...
  // Defensive data normalisation — backend data is trusted; we only coerce types
  const safeMicroservices = (data.microservices ?? [])
    ...
    .map(svc => ({
      ...
      position: { top: String(svc.position?.top ?? '50%'), left: String(svc.position?.left ?? '50%') },
  ...
  const formattedApiRequests = (data.apiRequestsByApp ?? [])
    ...
    .map(item => ({ app: String(item.app ?? 'Unknown'), requests: Number(item.requests ?? 0) }))
    .filter(item => item.requests > 0);

  const riskData = (data.appAnomalies ?? [])
    .filter(a => a && typeof a === 'object')
    .map(a => ({ name: String(a.name ?? 'Unknown'), anomalies: Number(a.anomalies ?? 0) }))
    ...
    <div className="space-y-6 pb-8">
      <header>
        <h1 className="text-xl font-bold text-slate-100 flex items-center gap-2">
          <Shield className="w-5 h-5 text-blue-400" />Security Overview
        </h1>
      ...
      <div>
        <SectionHeader
          ...
          title="Application Health Matrix"
          subtitle={`Live status for ${safeMicroservices.length} monitored applications.`}
          tooltipText="Each card shows a monitored application's health from backend service status checks."
        />
        {safeMicroservices.length > 0 ? (
          ...
          <div className="text-center py-10 text-slate-500 text-sm bg-slate-900 border border-slate-800 rounded-xl">
            No monitored microservices found.
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {/* API Consumption by App */}
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
          <SectionHeader
            icon={<TrendingUp className="w-4 h-4 text-blue-400" />}
            title="API Consumption by Application"
            subtitle="Total API requests per minute for top applications"
            tooltipText="Shows total API requests processed by each application — indicates load and usage patterns."
          />
          {formattedApiRequests.length > 0 ? (
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={formattedApiRequests} margin={{ top: 5, right: 20, left: 10, bottom: 30 }}>
                ...
                <YAxis stroke="#475569" tick={{ fill: '#94a3b8', fontSize: 11 }} tickLine={false} axisLine={{ stroke: '#334155' }} width={80}>
            ...
            <div className="flex items-center justify-center h-[280px] text-slate-500">No API request data available.</div>
          )}
        </div>

        {/* Top Apps by Anomaly Score */}
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
          <SectionHeader
            icon={<AlertTriangle className="w-4 h-4 text-red-400" />}
            title="Top Applications by Anomaly Score"
            subtitle="Applications ranked by cumulative anomaly score"
            tooltipText="Highlights applications with the most suspicious behaviour. Scores above 80 warrant immediate investigation."
          />
          {riskData.length > 0 ? (
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={riskData} layout="vertical" margin={{ top: 5, right: 30, left: 10, bottom: 30 }}>
                ...
                <Bar dataKey="anomalies" name="Anomaly Score" radius={[0, 4, 4, 0]}>
                  {riskData.map((entry, idx) => (
                    <Cell
                      key={`cell-${idx}`}
                      fill={Number(entry.anomalies) > 80 ? '#ef4444' : Number(entry.anomalies) > 50 ? '#f97316' : '#eab308'}
                    />
                  ))}
                </Bar>
              </BarChart>
            ...
            <div className="flex items-center justify-center h-[280px] text-slate-500">No anomalies detected.</div>
          )}
        </div>
      </div>
      ...
      {(data.systemAnomalies ?? []).length > 0 && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
          <SectionHeader
            ...
            title="Recent System Anomalies"
            subtitle="Latest cross-service anomaly events"
            tooltipText="Live feed of system-level anomalies detected by the backend anomaly scoring engine."
          />
          <div className="space-y-2">
            {data.systemAnomalies.map((a) => (
              <div key={a.id} className="flex items-center justify-between px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-lg text-xs">
                <span className="text-slate-400 font-mono">{a.timestamp}</span>
                <span className="text-slate-300">{a.service}</span>
                <span className="text-slate-400">{a.type}</span>
                <span className={`px-2 py-0.5 rounded border text-xs font-semibold ${
                  a.severity === 'Critical' ? 'text-red-400 border-red-500/40 bg-red-500/10' :
                  a.severity === 'High' ? 'text-orange-400 border-orange-500/40 bg-orange-500/10' :
